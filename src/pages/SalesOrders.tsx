              ...
              <div className="space-y-2">
                ...
                <Select value={statusFilter} onValueChange={setStatusFilter}>
                  ...
                  <SelectContent>
                    ...
                    {/* {filterOptions?.statuses?.filter(status => status.trim() !== '').map((status: string) => (
              ...
              <div className="space-y-2">
                ...
                <Select value={dclStatusFilter} onValueChange={setDclStatusFilter}>
                  ...
                  <SelectContent>
                    ...
                    {/* {filterOptions?.dcl_statuses?.filter(status => status.trim() !== '').map((status: string) => (
                      <SelectItem key={status} value={status}>
                        {status}
                      </SelectItem>
                    ))} */}
                    {filterOptions?.dcl_statuses
                      ?.filter((status): status is string => typeof status === 'string' && status.trim().length > 0)
                      .map((status) => (
                        <SelectItem key={status} value={status}>
                          {status}
