import { Package, TrendingUp, Calendar, RefreshCw, CheckCircle, Clock } from "lucide-react";
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Track purchase orders and inventory receipts.</p>
          </div>
        </div>
        ...
        <>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Card className="gradient-border-success">
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {orders.filter(o => o.katana_status === 'RECEIVED').length}
                </div>
            ...
            <Card className="gradient-border-warning">
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {orders.filter(o => o.katana_status === 'NOT_RECEIVED').length}
                </div>
            ...
            <Card className="gradient-border-primary">
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {orders.length > 0 
                    ? Math.round(orders.reduce((sum, order) => sum + order.fulfillment_percentage, 0) / orders.length)
                    : 0}%
                </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Track all purchase orders and their fulfillment status.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {orders.map((order) => (
                    <TableRow key={order._id}>
                      ...
                      <TableCell>
                        <Badge className={getStatusColor(order.katana_status)}>
                          {order.katana_status.replace('_', ' ')}
                        </Badge>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="font-medium">
                          {order.quantity_received.toLocaleString()}
                        </div>
                        <div className="text-sm text-muted-foreground">units</div>
                      </TableCell>
                      <TableCell>
                        <div className="space-y-2">
                          <div className="flex items-center justify-between text-sm">
                            <span>{Math.round(order.fulfillment_percentage)}%</span>
                          </div>
                          <Progress 
                            value={Math.min(order.fulfillment_percentage, 100)} 
                            className="h-2"
                          />
                        </div>
                      </TableCell>
